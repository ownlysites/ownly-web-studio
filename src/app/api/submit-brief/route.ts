...
type Payload = {
  ...
  industry?: string; budget?: string; current?: string; goal?: string; style?: string;
  honeypot?: string;
};
...
export async function POST(req: Request) {
  ...
  if (!row.email && !row.phone) {
    return NextResponse.json({ ok: false, error: "Need email or phone." }, { status: 422 });
  }
  ...
  if (error) {
    console.error("[submit-brief] insert failed", error);
    return NextResponse.json({ ok: true, persisted: false });
  }
